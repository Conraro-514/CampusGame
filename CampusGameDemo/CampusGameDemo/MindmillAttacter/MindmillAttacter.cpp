...
void MindmillAttacter(cv::Mat img_clone,cv::Mat img){
    ...
    cv::Scalar lower_circle(113,0,214);
    cv::Scalar upper_circle(180,255,255);
    cv::inRange(img_clone,lower,upper,mask_circle);//拟合圆
    //cv::imshow("mask",mask);
    //cv::waitKey(1);
    ...
    for(int i=0;i<contours.size();i++){
        ...
        circle_points.push_back(box.center);//储存最小外接矩形中心点
        cv::Point2d c;//圆心
        double r = 0;//半径
        LeastSquaresCircleFitting(circle_points, c, r);//拟合圆
        cv::circle(img, c, r, cv::Scalar(0, 0, 255), 2, 8);//绘制圆
        cv::circle(img, c, 5, cv::Scalar(255, 0, 0), -1, 8);//绘制圆心
